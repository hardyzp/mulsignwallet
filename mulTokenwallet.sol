...
contract MultiSigWallet {
    ...
    function signTransaction(uint transactionId) validOwner public {
        ...
        if (transaction.signatureCount >= MIN_SIGNATURES) {
            //check balance
            require(wowToken.balanceOf(address(this)) >= transaction.amount);            
            wowToken.transfer(transaction.to,transaction.amount);
            //emit an event
            emit transactionCompleted(address(this), transaction.to, transaction.amount, transactionId);
